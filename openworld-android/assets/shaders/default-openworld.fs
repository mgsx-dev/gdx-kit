...
varying float v_fog; // TODO remove v_fog ?
#endif // fogFlag
...
void main() {
	...
	#ifdef fogFlag
		float fogLinear = clamp(gl_FragCoord.z * gl_FragCoord.w, 0.0, 1.0);
		float fogQuadra = pow(1.0 - fogLinear, u_fogExponent);
		gl_FragColor.rgb = mix(gl_FragColor.rgb, u_fogColor.rgb, fogQuadra);
	#endif // end fogFlag
