...
public class GPUParticlesSystem extends EntitySystem
{
	...
	public GPUParticlesSystem(EditorScreen screen) {
	...
	public void addedToEngine(Engine engine) 
	{
		...
		for(int y=0 ; y<h ; y++){
			for(int x=0 ; x<w ; x++){
				int p = (y * w + x) * 4;
				vertices[p + 0] = x / (float)w;
				vertices[p + 1] = 0;
				vertices[p + 2] = y / (float)h;
		...
		shader = new ShaderProgram(Gdx.files.internal("gpu/particles.vs"), Gdx.files.internal("gpu/particles.fs"));
		
		int gs = loadShader(GL_GEOMETRY_SHADER, Gdx.files.internal("gpu/particles.gs").readString());
