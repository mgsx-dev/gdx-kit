...
public class AddSpriteAnimationTool extends RectangleTool 
{
	private Sprite sprite;
	private TextureRegion region;
	private FileHandle selectedFile;
	
	public AddSpriteAnimationTool(EditorScreen editor) {
		super("Sprite Animation", editor);
	}
	...
	protected void create(Vector2 startPoint, Vector2 endPoint) {
		...
		if(sprite != null){
			...
			// autodetect animations
			String baseName = selectedFile.nameWithoutExtension().replaceFirst("(.*)[0-9]+$", "$1");
			String ext = selectedFile.extension();
			if(selectedFile.nameWithoutExtension().endsWith("1")){
				if(selectedFile.parent().child(baseName + "2." + ext).exists()){
					Array<Sprite> sprites = new Array<Sprite>();
					for(int i=1 ; ; i++){
						FileHandle file = selectedFile.parent().child(baseName + i + "." + ext);
						if(!file.exists()){
							
							break;
						}
						sprites.add(new Sprite(editor.loadAssetNow(file.path(), Texture.class)));
					}
					Animation2DComponent anim = getEngine().createComponent(Animation2DComponent.class);
					anim.animation = new Animation<Sprite>(1, sprites);
					anim.time = 0;
					entity.add(anim);
				}
			}
