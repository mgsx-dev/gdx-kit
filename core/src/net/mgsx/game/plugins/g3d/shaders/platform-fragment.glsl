// copied from https://gist.github.com/xoppa/9766698
...
void main() {
	...
	// TODO very good shader !

	vec3 lightFactor = saturate((v_lightCol * diffuse.rgb) * NL) + v_ambientLight * diffuse.rgb + (selfShadow * spec) * specular.rgb;
	float backLightFactor = 1.0-dot(N.xyz, L);

	float sh = 1.0 - saturate(V.z);
	sh = pow(sh, 1.0) * backLightFactor;
