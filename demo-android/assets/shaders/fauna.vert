...
//	v_alpha = rateA * rateB;
	...
	float finZ = (0.2+sin((sin(seed2d.x + 54.567 + u_time) + sin(seed2d.y + 54.567 + u_time)) * 0.1)) * 4;

	v_position = vec3(
			finX,
			(1.1 + sin(u_time + seed2d.x + seed2d.y)) * 1.01 + 4,
			finY);


	vec4 screen_pos = u_projTrans * vec4(v_position, 1.0);

	v_alpha = smoothstep(0,1, (screen_pos.z * screen_pos.w - 130) / 30);

	float size = 1.0 / (4 + screen_pos.z);
